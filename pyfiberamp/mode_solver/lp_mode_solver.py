...
ModeSolverResult = namedtuple('Result', 'root converged')
...
class LPModeSolver:
    ...
    def check_lm(self, l: int, m: int):
        if l > self.l_max:
            raise ModeNotFoundError(f'l = {l} exceeds l_max = {self.l_max}')
        if m > self.m_max:
            raise ModeNotFoundError(f'm = {m} exceeds m_max = {self.m_max}')
    ...
    def _create_cutoff_table(l_max: int, m_max: int):
        ...
        l_max and m_max must be chosen so that """
    ...
    def solve_for_u(self, l: int, m: int, v: float):
        ...
        if start >= stop:
            return ModeSolverResult(root=0, converged=False)
        else:
            ...
            return ModeSolverResult(root=root, converged=res.converged)
    ...
    def find_mode(self, l: int, m: int, core_radius: float, na: float, wl: float, rotation='cos'):
        ...
        return pyfiberamp.mode_solver.lp_mode.LPMode(l, m, res.root, na, core_radius, wl, rotation)
...
if __name__ == '__main__':
    ...
    mode_solver.find_mode(0, 2, 10e-6, 0.065, 1064e-9)
