...
class YbDoubleCladTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.fiber = YbDopedDoubleCladFiber(length=1, core_radius=r, core_na=0.12, ion_number_density=nt,
                                       ratio_of_core_and_cladding_diameters=10, background_loss=cls.default_loss)
        cls.pump_power = 0.5
    ...
    def test_steady_state(self):
        ...
        steady_state_simulation.add_cw_signal(wl=self.signal_wl, power=self.signal_power, loss=self.signal_loss)
        steady_state_simulation.add_forward_pump(wl=self.pump_wl, power=self.pump_power)
        steady_state_simulation.add_backward_pump(wl=self.pump_wl, power=self.pump_power, loss=self.bw_pump_loss)
